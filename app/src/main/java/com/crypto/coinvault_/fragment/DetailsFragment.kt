...
import com.crypto.coinvault_.databinding.FragmentDetailsBinding
import com.crypto.coinvault_.models.CryptoCurrency
...
class DetailsFragment : Fragment() {
   ...
   private  val item : DetailsFragmentArgs by navArgs()
    ...
    private fun readData(){
        val sharePreferences = requireContext().getSharedPreferences("watchlist", Context.MODE_PRIVATE)
        val gson =Gson()
        val json = sharePreferences.getString("watchlist", ArrayList<String>().toString())
    ...
    private fun setButtonOnClick(item: CryptoCurrency){
        ...
        val fifteenMinut = binding.button5

        val clickListener = View.OnClickListener{
            when(it.id){
                fifteenMinut.id -> loadChartData(it, "15", item, oneDay, oneMonth, oneWeek, fourHour, oneHour)
                oneHour.id -> loadChartData(it, "1H", item, oneDay, oneMonth, oneWeek, fourHour, fifteenMinut)
                fourHour.id -> loadChartData(it, "4H", item, oneDay, oneMonth, oneWeek, fifteenMinut, oneHour)
                oneDay.id -> loadChartData(it, "D", item, fifteenMinut, oneMonth, oneWeek, fourHour, oneHour)
                oneWeek.id -> loadChartData(it, "W", item, oneDay, oneMonth, fifteenMinut, fourHour, oneHour)
                oneMonth.id -> loadChartData(it, "M", item, oneDay, fifteenMinut, oneWeek, fourHour, oneHour)
            }

        }

        fifteenMinut.setOnClickListener(clickListener)
    ...
    private fun loadChartData(
        ...
        binding.detaillChartWebView.loadUrl(
            ...
            "https://s.tradingview.com/widgetembed/?frameElementId=tradingview_76087&symbol=" + item.symbol
